    ...
    public function run() {
        ...
        foreach ($projects as $project) {
            ...
            foreach ($projectTaskGroups as $taskGroup) {
                ...
                Task::factory()
                    ->count($faker->numberBetween(TaskSeeder::MIN_TASKS_PER_GROUP, TaskSeeder::MAX_TASKS_PER_GROUP))
                    ->state(new Sequence(
                        fn ($sequence) => ['position' => $sequence->index + 1]
                    ))
                    ->hasAttached(
                        $projectMembers->random($faker->numberBetween(
                    ...
                    ->hasAttached(
                        $projectTags->random($faker->numberBetween(
