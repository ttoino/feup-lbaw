...
export const completeTest = (taskId: string) => fetch(`/api/task/${taskId}/complete`, {
    method: "PUT",
});

export const repositionTask = (taskId: string, task_group: string | null, position: string | null) => fetch(`/api/task/${taskId}/reposition`, {
    method: "POST",
    body: JSON.stringify({
        task_group,
        position
