...
<li class="thread-comment editable"
    ...
    @can('edit', $project)
        <div @class(['hstack', 'gap-2', 'd-none' => !$threadComment->editable])
        ...
        <form class="edit-thread-comment-form edit needs-validation" novalidate>
            <input type="hidden" name="id" value="{{ $threadComment->id }}"
                data-render-value="id">

            <div class="form-floating">
                <textarea placeholder="" class="form-control auto-resize"
                    id="thread-comment-content"
                    aria-describedby="thread-comment-content-feedback" name="content"
                    minlength=6 maxlength=512 required data-render-value="content.raw">{{ $threadComment->content['raw'] }}</textarea>
                <a href="https://www.markdownguide.org/basic-syntax/"><i
                        class="bi bi-markdown"></i> Markdown is supported</a>
                <label for="thread-comment-content"
